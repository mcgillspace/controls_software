...
C_r = (2*(eta_r^2)-1)*eye(3) + 2*qr*eps_Tr - 2*eta_r*eps_xr;
rotation_mat = (2*(eta^2) - 1)*eye(3) + 2*eps*(eps.') - 2*eta*crossop(eps);
C_e = C_r*inv(rotation_mat);
...
disturb = a;
%disturb = 0;
disturbance = [0; disturb; 0];
%disturbance = [10e-9; 10e-9; 10e-9];
...
%PID control
r = [altitude*cos((2*pi/orbit_period)*t), altitude*sin((2*pi/orbit_period)*t)*cos(incl), altitude*sin((2*pi/orbit_period)*t)*sin(incl)];
...
disturbance_body = rotation_mat * disturbance;
